...
enum class overlay_mode { hidden, search, rsearch, completion, urlselect };

enum class vi_mode { insert, command, visual, visual_line, visual_block };
...
struct search_panel_info {
  ...
  VteRegex* last_regex{nullptr};
  std::string last_pattern{};

  ~search_panel_info() {
    g_clear_object(&entry);
    g_clear_object(&da);
    if (last_regex) vte_regex_unref(last_regex);
  }
};
...
static gboolean window_state_cb(GtkWindow* window, GdkEventWindowState* event,
                                keybind_info* info);
static gboolean key_press_cb(VteTerminal* vte, GdkEventKey* event,
                             keybind_info* info);
static gboolean entry_key_press_cb(GtkEntry* entry, GdkEventKey* event,
                                   keybind_info* info);
static gboolean position_overlay_cb(GtkBin* overlay, GtkWidget* widget,
                                    GdkRectangle* alloc);
...
static void search(VteTerminal* vte, search_panel_info* panel,
                   const char* pattern, bool reverse);
static void overlay_show(search_panel_info* info, overlay_mode mode,
                         VteTerminal* vte, int completion_limit = 5000);
...
static void launch_in_directory(VteTerminal* vte) {
  const char* uri = vte_terminal_get_termprop_string_by_id(
      vte, VTE_PROPERTY_ID_CURRENT_DIRECTORY_URI, nullptr);
  if (!uri) {
    g_printerr("no directory uri set\n");
    return;
  }
  auto dir =
      make_unique_ptr(g_filename_from_uri(uri, nullptr, nullptr), g_free);
  char term[] = "termite";  // maybe this should be argv[0]
  char* cmd[] = {term, nullptr};
  g_spawn_async(dir.get(), cmd, nullptr, G_SPAWN_SEARCH_PATH, nullptr, nullptr,
                nullptr, nullptr);
}

static void find_urls(VteTerminal* vte, search_panel_info* panel_info) {
  static GRegex* regex = g_regex_new(
      url_regex, (GRegexCompileFlags)(G_REGEX_CASELESS | G_REGEX_OPTIMIZE),
  ...
  for (unsigned row = 0; *start_pos != '\0';
    ...
    if (error) g_printerr("error while matching: %s\n", error->message);
  }
}

static void launch_url(char* browser, const char* text,
...
static gboolean draw_cb(const draw_cb_info* info, cairo_t* cr) {
  if (!info->panel->url_list.empty()) {
    ...
    size_t len =
        info->panel->fulltext == nullptr ? 0 : strlen(info->panel->fulltext);
    ...
    for (unsigned i = 0; i < info->panel->url_list.size(); i++) {
      ...
      if (len) active = strncmp(buffer, info->panel->fulltext, len) == 0;
...
static void update_selection(VteTerminal* vte, const select_info* select) {
  ...
  vte_terminal_set_selection_block_mode(vte,
                                        select->mode == vi_mode::visual_block);
  ...
  if (select->mode == vi_mode::visual) {
    ...
    if (begin < end) {
      ...
      vte_terminal_select_text(vte, cursor_col, cursor_row, selection_x_end,
                               select->begin_row);
    }
  } else if (select->mode == vi_mode::visual_line) {
    selection_x_end = n_columns;
    vte_terminal_select_text(vte, 0, std::min(select->begin_row, cursor_row),
                             selection_x_end,
...
static void open_selection(char* browser, VteTerminal* vte) {
  ...
  if (browser) {
    auto selection = make_unique_ptr(vte_terminal_get_selection(vte), g_free);
...
static void set_cursor_column(VteTerminal* vte, const select_info* select,
                              long column) {
  long cursor_row;
  vte_terminal_get_cursor_position(vte, nullptr, &cursor_row);
...
void window_title_cb(VteTerminal* vte, gboolean* dynamic_title) {
  const char* const title = *dynamic_title
                                ? vte_terminal_get_termprop_string_by_id(
                                      vte, VTE_PROPERTY_ID_XTERM_TITLE, nullptr)
                                : nullptr;
...
gboolean key_press_cb(VteTerminal* vte, GdkEventKey* event,
  ...
  if (info->select.mode != vi_mode::insert) {
    ...
    switch (event->keyval) {
      ...
      case GDK_KEY_u:
        for (long i = get_count(&info->select); i > 0; i--) {
          search(vte, &info->panel, url_regex, false);
        }
        break;
      case GDK_KEY_U:
        for (long i = get_count(&info->select); i > 0; i--) {
          search(vte, &info->panel, url_regex, true);
        }
        break;
      case GDK_KEY_o:
  ...
  if (modifiers == (GDK_CONTROL_MASK | GDK_SHIFT_MASK)) {
    switch (gdk_keyval_to_lower(event->keyval)) {
      ...
      case GDK_KEY_l:
        ...
        return TRUE;
      default:
        if (modify_key_feed(event, info, modify_table)) return TRUE;
    }
  } else if ((modifiers == (GDK_CONTROL_MASK | GDK_MOD1_MASK)) ||
             (modifiers ==
              (GDK_CONTROL_MASK | GDK_MOD1_MASK | GDK_SHIFT_MASK))) {
    if (modify_key_feed(event, info, modify_meta_table)) return TRUE;
    ...
    case GDK_KEY_Return: {
      ...
      switch (info->panel.mode) {
        case overlay_mode::search:
          search(info->vte, &info->panel, text, false);
          break;
        case overlay_mode::rsearch:
          search(info->vte, &info->panel, text, true);
          break;
...
gboolean button_press_cb(VteTerminal* vte, GdkEventButton* event,
  ...
  if (info->clickable_url && event->type == GDK_BUTTON_PRESS) {
    ...
    auto match = make_unique_ptr(
        vte_terminal_hyperlink_check_event(vte, (GdkEvent*)event), g_free);
    if (!match) {
      match = make_unique_ptr(check_match(vte, event), g_free);
    }
...
GtkTreeModel* create_completion_model(VteTerminal* vte, int completion_limit) {
  ...
  std::string_view sv(content.get());
  ...
  while (start < sv.size()) {
    ...
    if (end > start) {
      tokens.insert(sv.substr(start, end - start));
    }
    start = end + 1;
  }

  std::vector<std::string_view> sorted_tokens(tokens.begin(), tokens.end());
...
void search(VteTerminal* vte, search_panel_info* panel, const char* pattern,
            bool reverse) {
  auto terminal_search = reverse ? vte_terminal_search_find_previous
                                 : vte_terminal_search_find_next;

  if (panel->last_pattern != pattern) {
    if (panel->last_regex) {
      vte_regex_unref(panel->last_regex);
      panel->last_regex = nullptr;
    }
    ...
    if (panel->last_regex) {
      vte_regex_jit(panel->last_regex, PCRE2_JIT_COMPLETE, nullptr);
      vte_regex_jit(panel->last_regex, PCRE2_JIT_PARTIAL_SOFT, nullptr);
    }
    panel->last_pattern = pattern;
  }

  vte_terminal_search_set_regex(vte, panel->last_regex, 0);
...
void overlay_show(search_panel_info* info, overlay_mode mode, VteTerminal* vte,
  ...
  if (vte) {
    ...
    GtkTreeModel* completion_model =
        create_completion_model(vte, completion_limit);
...
static char* get_user_shell_with_fallback() {
  ...
  if (char* command = vte_get_user_shell()) {
    if (!((command != NULL) && (command[0] == '\0'))) return command;
  }
...
static void spawn_callback(VteTerminal* terminal, GPid pid, GError* error,
                           gpointer user_data) {
  if (!terminal) {
    return;
  }
...
static void drag_data_received_cb(GtkWidget*, GdkDragContext*, int, int,
                                  GtkSelectionData* data, unsigned, unsigned,
                                  VteTerminal* vte) {
  std::string selection{
      reinterpret_cast<const char*>(gtk_selection_data_get_text(data))};
...
int main(int argc, char** argv) {
  ...
  if (version) {
    g_print("termite %s\n", TERMITE_VERSION);
    g_print("vte %u.%u.%u %s\n", VTE_MAJOR_VERSION, VTE_MINOR_VERSION,
            VTE_MICRO_VERSION, vte_get_features());
    return EXIT_SUCCESS;
  }
  ...
  keybind_info info{
      GTK_WINDOW(window),       vte, search_panel_info{}, select_info{},
      config_info{config_file},
  };

  load_config(info.config, vte, GTK_WINDOW(window), scrollbar, hbox,
              icon ? nullptr : &icon, &show_scrollbar);

  reload_config = [&] {
    load_config(info.config, vte, GTK_WINDOW(window), scrollbar, hbox, nullptr,
                nullptr);
  };
  g_unix_signal_add(
      SIGUSR1,
  ...
  g_signal_connect_swapped(
      ...
      G_CALLBACK(+[](keybind_info* info, GdkEventCrossing* event) -> gboolean {
  ...
  g_signal_connect_swapped(
      ...
      G_CALLBACK(+[](keybind_info* info, GdkEventConfigure* event) {
        if (info->config.dpi_aware)
          adjust_font_size(info->vte, event->window, info->config.font_size);
      }),
      &info);
  ...
  g_signal_connect_swapped(
      vte, "selection-changed", G_CALLBACK(+[](keybind_info* info) {
